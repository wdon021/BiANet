...
import pdb_graph_pe as pdb_graph
import mol_graph_pe as mol_graph
import pickle
...
def create_full_ood_set(df, fold_seed, frac):
    ...
    """
    train_frac, val_frac, test_frac = frac
    test_drugs = df['CID'].drop_duplicates().sample(frac=test_frac, replace=False, random_state=fold_seed).values
    test_prots = df['Kinase'].drop_duplicates().sample(frac=test_frac, replace=False, random_state=fold_seed).values

    test = df[(df['CID'].isin(test_drugs)) & (df['Kinase'].isin(test_prots))]
    train_val = df[(~df['CID'].isin(test_drugs)) & (~df['Kinase'].isin(test_prots))]
...
class DAVIS(DTATask):
    ...
    """
    def __init__(self,
            data_path='.\DAVIS\davis_data.tsv',            
            pdb_map='.\DAVIS\davis_protein2pdb.yaml',
            pdb_json='.\structure\pockets_structure.json',                        
            emb_dir='.\data\data\esm2',           
            num_pos_emb=16, num_rbf=16,
            contact_cutoff=8.,            
            drug_sdf_dir='.\data\structure\davis_mol3d_sdf',
            split_method='random', split_frac=[0.7, 0.1, 0.2],
            mmseqs_seq_cluster_file='.\data\data\DAVIS\davis_cluster_id50_cluster.tsv',
            seed=42, onthefly=False
        ):
...
if __name__ == "__main__":
    class Args:
        def __init__(self):
            ...
            self.task = "DAVIS"  # Make sure this is explicitly set
            self.split_method = "seqid" # Make sure this is explicitly set
            self.split_frac = [0.7, 0.1, 0.2]
            self.k_shot = 24
            self.k_query = self.k_shot*2
            self.batch_size = 1
            self.val_shot = 50
            self.seed =55+48
            ...
            self.num_pos_emb = 16
            self.num_rbf = 16
            self.contact_cutoff = 8.0
            self.onthefly = False
            self.positive_cutoff = 3000
            self.adaptive = False
            self.left_over_size = 6
            self.num_workers = 2
            self.base_path = "DTI Project/baseModel/Sigmoid_cross_attention/"
            self.data_path = '.\DAVIS\davis_data.tsv'
            self.pdb_map = '.\DAVIS\davis_protein2pdb.yaml'
            self.pdb_json = '.\structure\pockets_structure.json'
            self.emb_dir = '.\data\data\esm2'
            self.drug_sdf_dir = '.\data\structure\davis_mol3d_sdf'
            self.mmseqs_seq_cluster_file = 'davis_cluster_id50_cluster.tsv'

# Initialize arguments
args = Args()

folders = ["split2", "split3", "split4", "split5"]
    
    
    
# Store original base path
original_base_path = args.base_path

for fold, split in enumerate(folders):
    print(f"Processing fold {fold}: {split}")

    # Use original base path, not the modified one
    current_path = os.path.join(original_base_path, split)
    os.makedirs(current_path, exist_ok=True)

    args.seed = 42 + fold  # Use fold (int) not split (string)

    dataset = DAVIS(
        num_pos_emb=16, num_rbf=16,
        contact_cutoff=8.,
        split_method=args.split_method, split_frac=args.split_frac,
        seed=args.seed, onthefly=args.onthefly,
        data_path=args.data_path,            
        pdb_map=args.pdb_map,
        pdb_json=args.pdb_json,                        
        emb_dir=args.emb_dir,                
        drug_sdf_dir=args.drug_sdf_dir,
        mmseqs_seq_cluster_file=args.mmseqs_seq_cluster_file,
    )
    print("Saving Davis encoder...")
    # encoder_path = 'C:\\PhD_projects\\DTI project\\saved_davis_encoder.pkl'
    # encoder_path = os.path.join(args.base_path, 'saved_DAVIS_S3_encoder.pkl')
    # with open(encoder_path, 'wb') as f:
    #         pickle.dump(dataset, f)
    data_splits, df_splits = dataset.get_split(return_df=True)

    print("Saving DAVIS data splits...")
    splits_path = os.path.join(current_path, 'davis_S3_splits.pkl')
    with open(splits_path, 'wb') as f:
        pickle.dump((data_splits, df_splits), f)

    # Also save DataFrame splits as CSV for easier inspection/loading
    for split_name, df in df_splits.items():
        df.to_csv(os.path.join(current_path, f'DAVIS_{split_name}_S3_split.csv'), index=False)
